...
This file is calibration, not another workflow step to run through in order. Sampling for variety alone does not stop convergence — a stance can be freshly rolled and still land on a trained-in default, because those defaults are what the underlying models reach for absent a reason not to. Nothing named below is forbidden on sight: when a brief genuinely asks for one of these looks, build it, and build it well. The failure mode this file exists to catch is different — one of these appearing because it's the reflex answer, not because the brief called for it, or surviving into a shipped page after the brief was silent on the question. Grafted with attribution from two outside sources: Anthropic's `frontend-design` skill (terms in its own LICENSE.txt) and `impeccable` v3.5 (Apache 2.0).
...
1. A saturated brand color as the body itself — terracotta, oxblood, deep ochre, near-black — rather than a near-white stand-in for one.
2. A true off-white at chroma 0, or tinted toward the brand's own hue rather than toward warmth by default.
...
Match-and-refuse: if the current build is about to produce any of these, stop and rebuild the element with different structure rather than tone it down.
...
- **An eyebrow above every section.** A small tracked all-caps label sitting above every heading on the page. Keep at most one deliberate kicker as a real brand device; an eyebrow on every section is scaffolding, not voice.
- **Numbered markers as reflex scaffolding.** `01 / 02 / 03` placed above sections that aren't actually an ordered sequence. Reserve numbers for content that is a real process, flow, or timeline, where the order itself carries information the reader needs.
...
**First-order.** If someone could guess the palette and theme from the product's category alone — a wellness app defaulting to sage and cream, a fintech product defaulting to navy and gold — that's the first training-data reflex firing. Rework the color strategy and the physical-scene sentence below until the category by itself no longer predicts the answer.
...
**Dark vs. light, decided by scene, not by default.** Neither is the safe choice. Before picking, write one sentence describing the actual physical scene: who is looking at this, in what place, under what light, in what frame of mind. If that sentence doesn't force an answer on its own, it isn't concrete enough yet — add detail until it does.

**Color-strategy axis.** Pick a commitment level before picking any colors:

- **Restrained** — tinted neutrals plus one accent used sparingly, at or below roughly 10% of the surface. The default for product minimalism.
- **Committed** — one saturated color carries somewhere around 30–60% of the surface. The default for identity-driven pages.
- **Full palette** — three to four named color roles, each with a deliberate job. Fits brand campaigns and product data visualization.
- **Drenched** — the surface itself is the color. Reserved for brand hero moments and campaign pages.
...
**No aphoristic-cadence voice as the page's default rhythm.** Watch for a serious statement followed by a short, punchy negation, repeated section after section until it becomes the house voice. If three or more copy blocks on one page land on that same short-rebuttal shape, rewrite toward language specific to what's actually being said there.

**No buzzword family.** Streamline, empower, supercharge, leverage, unleash, transform, seamless, world-class, enterprise-grade, next-generation, cutting-edge, game-changer, mission-critical. Swap each for a concrete noun and a verb naming what the product literally does.
