...
7. **Content pass.** No lorem ipsum, numbers are internally consistent, sibling copy is the same length, sentence case is held. Every image slot is filled — a `src` the imagery ladder verified or a project asset, alt text, intrinsic size, a container colour, and the credit its source requires — and none is a guessed URL.
