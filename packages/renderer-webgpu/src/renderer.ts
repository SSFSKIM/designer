  ...
  function drawGroups(
    ...
    for (const entry of groups.values()) {
      ...
      passes.opticsPass(encoder, {
        ...
        //
        // Under the accessibility fold it stands down (W31 Decision Log 3 (d);
        // claims §5.164 §8 (b) and §13): an occlusion lift covers more of the
        // backdrop, so restoring the nominal fraction of its chromaticity gives
        // back what the preference asked to have covered — measured at `R` 3.05
        // and 3.17 against a reference of 1. `bodyChromaRetentionUnderPolicy` is
