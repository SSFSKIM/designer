...
- 93136b3: Correct the macOS 27 material's black-backdrop level in both window poses. A compact response
  branch matches the measured native black body—132/133 codes in light mode and 32/20 in dark
  mode—while rejoining the previous response below the canonical impulse inputs. The four macOS 27
  endpoint fingerprints change; the selectable macOS 26.5 material and its fingerprints do not.
  
  The CSS tier derives the same response, with an explicitly measured +1/+2-code active boundary
  residual. The grey middle, chroma and rim gaps are not claimed closed. The separate receded colour
  formula was tested and declined at the existing structured regression stops. Evidence and scope
  are recorded in fidelity claims §5.179.

### Patch Changes

- @vitreajs/vitrea@0.24.0

## 0.23.0

### Minor Changes

- 6f3435d: Stand down the exterior lift on the macOS 27 material, and compensate its thick-shadow anchors.
...
- 742e244: **The outer shadow's exterior is fitted as a whole, and a receded window stops casting one.**
...
- @vitreajs/vitrea@0.22.0

## 0.21.0

### Minor Changes

- d580cb8: Carry the backdrop's hues through the body, and stop a leaf added at its
...
- @vitreajs/vitrea@0.21.0

## 0.20.0

### Minor Changes

- 44b43d5: Grade the outer shadow's blur by the casting surface, and condition the dark
...
- d9a4937: Tell the React bindings which material document the root selected, so a toolbar
...
- 8aaf02d: The ink is Apple's own label colour now, through Apple's own vibrancy operator.
...
- ad904d5: Allow material profile patches to carry a three- or four-knot backdrop response and an optional `increasedOcclusionLiftByPolicy` map for distinct Reduce Transparency and Increase Contrast levels. Both additions preserve the shared, three-knot resting defaults when omitted.
- @vitreajs/vitrea@0.17.0
