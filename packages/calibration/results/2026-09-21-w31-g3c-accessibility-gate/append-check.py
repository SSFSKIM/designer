...
The clauses, each reported on its own line:
  ...
  counts add up        per profile key, the rows ON DISK in each file are the rows the
                       manifest sends it, and they add back up to the before count
  reconstruction       the recomposed original hashes to the recorded digest

**FIVE of the six clauses read the files, not the plan** (W30 G1 review closure,
2026-09-20; the fifth added by W31 G3c, claims §5.164 §13). A clause computed from the
manifest alone proves that the split *intended* the right thing; the question is whether
the tree *holds* it, and the clauses that name the invariants most worth holding — the
frozen bed and the order the freeze reads positionally — take the rows off disk and
compare them to the recorded digests.

**"Counts add up" was the sixth, and it could not fail.** It built one dict from the
manifest — incrementing `before` and `destination` in the same loop over the same rows —
and then asserted that the destinations sum to `before`, which is `n == n` for every
profile key on every possible input. It read "PASS" in W31 G3's `append-check.txt` and in
every earlier gate's, and it would have read PASS over a tree where the split had written
nothing at all. It now counts the rows each file HOLDS, by the profile key each row itself
carries, and compares that to the manifest per file and per key — so a file short of a row
fails on the count as well as on the digest sequence, and the clause says something the
other five do not say in the same words.
...
def main() -> int:
    ...
    # --- counts add up --------------------------------------------------------------
    # W31 G3c (claims §5.164 §13): off the FILES. `planned` is what the manifest
    # sends each file, per profile key; `held` is what each file on disk carries,
    # by the profile key each row itself states. The clause is the equality of
    # the two, plus the sum over files returning the before count — which is a
    # statement about the tree and can fail.
    planned: dict[str, dict[str, int]] = {}
    for r in before:
        entry = planned.setdefault(r["profileKey"], {"before": 0})
        entry["before"] += 1
        entry[r["destination"]] = entry.get(r["destination"], 0) + 1
    held: dict[str, dict[str, int]] = {}
    for rel, holder in files.items():
        for key, _digest in holder["onDisk"]:
            held.setdefault(key, {})[rel] = held.setdefault(key, {}).get(rel, 0) + 1
    counts = {}
    ok_counts = True
    for key in sorted(set(planned) | set(held)):
        want = {k: v for k, v in planned.get(key, {}).items() if k != "before"}
        got = held.get(key, {})
        counts[key] = {
            "before": planned.get(key, {}).get("before", 0),
            "planned": dict(sorted(want.items())),
            "onDisk": dict(sorted(got.items())),
        }
        if want != got:
            ok_counts = False
            failures.append(
                f"{key}: the manifest sends {want} and the files hold {got}")
        elif sum(got.values()) != planned.get(key, {}).get("before", 0):
            ok_counts = False
            failures.append(
                f"{key}: {sum(got.values())} rows on disk against a before count of "
                f"{planned.get(key, {}).get('before', 0)}")
    report["clauses"]["counts add up"] = ok_counts
    report["rowsByProfileKey"] = counts
    ...
    print()
    # The counts, ON DISK, beside what the manifest planned (W31 G3c; finding
    # N17). Two columns per file rather than one, because "they agree" is the
    # clause and the agreement is what a reader should be able to see.
    print(f"{'profile key'.ljust(58)} {'before':>7} {'working':>8} {'moved':>7}  planned == on disk")
    for pk, v in report["rowsByProfileKey"].items():
        on_disk = v["onDisk"]
        moved = sum(n for k, n in on_disk.items() if k != "results/matrix.json")
        agree = "yes" if v["planned"] == on_disk else "NO"
        print(f"{pk.ljust(58)} {v['before']:>7} {on_disk.get('results/matrix.json', 0):>8} "
              f"{moved:>7}  {agree}")
