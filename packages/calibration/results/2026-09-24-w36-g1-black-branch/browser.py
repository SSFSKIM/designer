"""W36 one scratch launch per profile, with W35 admission and X6 (§5.179 clauses 2, 4–7).
...
SCRATCH=Path.home()/'vitrea-w36/scratch/black-price'
def dump(path,value):
...
def prepare():
    ...
    fixtureRoot=SCRATCH/'native-inputs';fits=json.loads((HERE/'candidate-ordinates.json').read_text())
    for profile in profiles:
        ...
        act=dict(profileKey=profile,patch=active['patch'],claims='c9a §5.179; unsealed black-only price')
        ...
        rec['patch'].update(inactive['patch']);rec['claims']='c9a §5.179; explicit receded black ordinate over active tune'
        ...
        plans.append(dict(profile=profile,scenes=admitted,root=str(work),command=command,environment=environment,
            active=act,receded=rec,boundsSha256=hashlib.sha256((HERE.parent/'2026-09-24-w36-g0-level-cut/bounds-declaration.md').read_bytes()).hexdigest()))
...
def render():
    ...
    for plan in json.loads((HERE/'candidate-plans.json').read_text()):
        ...
        old.log(dict(label='black-price-'+profile,checks=checks,repeatFailures=sum(not c['used'] for c in checks)))
