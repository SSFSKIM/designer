#!/usr/bin/env python3
"""W36 G1 — the READ's append-check: the canonical read appended and rewrote nothing.
...
def snapshot(source: pathlib.Path | None = None) -> int:
    ...
    SNAPSHOT.write_text(
        json.dumps(
            {
                "$comment": [
                    "results/matrix.json as it stood BEFORE W36 G1's canonical read",
                    "(claims §5.179). Written once, never rewritten.",
                    "Recovered from seal commit 2f49d390; its SHA equals the live pre-read before-read.json.",
                ],
