...
"""The sealed W36 read, one artifact for holdout, X6 before every browser (§5.179).
...
def main():
    ...
    configuration=load('w36_configuration',CAL/'results/holdout-configuration/configuration.py')
    assert configuration.document_hashes()=={n:e['fileSha256'] for n,e in sealed['documents'].items()}
    reads=configuration.load_log()
    assert reads[-1]['claims']=='c9a §5.179'
    ...
    x6=load('w36_capture',HERE.parent/'2026-09-24-w35-g0-edge-cut/browser.py')
    x6.HERE=HERE
    original_log=x6.log
    current_renderer='webgpu'
    def log(row):
        if 'renderer' in row:row={**row,'renderer':current_renderer}
        original_log(row)
    x6.log=log
    for profile,renderer in jobs:
        ...
        x6.preflight(f'canonical/{a.mode}/{profile}/{renderer}')
        ...
        result=subprocess.run(cmd,cwd=CAL,env=dict(os.environ,
            VITREA_WEB_CAPTURES=str(CAL/'web-captures'),VITREA_SCENE_SERVER_PORT='5197'))
        x6.log(dict(label=f'canonical/{a.mode}/{profile}/{renderer}',renderer=renderer,
            exitCode=result.returncode,command=cmd))
        if result.returncode:raise RuntimeError('Canonical pass failed; no implicit retry')
