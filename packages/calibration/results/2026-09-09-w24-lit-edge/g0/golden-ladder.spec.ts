/**
 * W24 G0 (c) — the goldens' attribution at every ladder point (X2).
 *
...
const POINTS: ReadonlyArray<readonly [string, object]> = [
  ["shipped (empty patch)", {}],
  ["rimLitExponent 1", { optics: { regular: { rimLitExponent: 1 } } }],
  ["rimLitExponent 2", { optics: { regular: { rimLitExponent: 2 } } }],
  // The axis alone, at the exponent the mechanism is inert at: it must move NOTHING, which is the
  // proof that the axis cannot reach a pixel on its own and that `rimLitExponent` is the whole gate.
  ["rimLitAxis rotated, exponent 0", { rimLitAxis: [-0.9239, -0.3827] }],
];
...
test.describe("@golden W24 the lit edge's reach on every golden", () => {
  test("every ladder point moves the contour band and nothing else", async ({ page }) => {
    ...
    say("W24 G0 — the lit edge's reach on every golden, per ladder point");
    say("");
    ...
    say("");
    say("The SHIPPED point is the empty patch and it is the golden suite's own bytes — proven");
    say("separately by the 31 pinned goldens and by the isolation proof's own hashes, both green");
    say("in this worktree with the mechanism in place and its exponent at 0.");
    say("");
    say(
