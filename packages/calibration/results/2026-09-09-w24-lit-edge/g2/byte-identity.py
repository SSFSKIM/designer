"""W24 G2 — every capture of the dry run against the canonical W23 bed, byte for byte.

This is stop S10 read on the bed: no cell that neither mechanism reaches may move at all. Both
mechanisms reach the CSS tier — the transmission through `adaptedSourceOptics`'s alpha and the lit
edge through `interiorBandLight`'s arc integral — so that tier's captures are expected to move, and
each mover has to be named rather than assumed.

Every capture on both tiers is compared. The GPU tier's movers are the lit edge (every cell that
draws a rim) and the transmission (the collapsed cells over a structured backdrop); the CSS tier's
are the same two through the mirror.
...
def main():
    ...
    emit("W24 G2 — the dry run's captures against the canonical W23 bed, byte for byte")
    emit("=" * 100)
