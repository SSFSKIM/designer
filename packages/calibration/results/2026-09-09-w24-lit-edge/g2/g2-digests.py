"""W24 G2 — the rows the landing must reproduce byte for byte (contract X3).

W21 G1's script, carried through W23 G1 to this wave's document. The dry run is the wave's one read of the canonical
...
BOTH profile documents move in this wave and both are recorded: the light one carries the lit
edge's exponent and axis and the collapse's two transmission anchors, and the dark one — whose own
patch does not move at all, both mechanisms being the material's — carries the same four through
the default it inherits. Their digests are printed beside the captures so a G2 that reproduces
these bytes can say WHICH documents it reproduced them from, out of this file rather than out of a
commit message.

    g2-digests.py <dry-run captures dir> <dry-run matrix.json> > g2-digests.txt
"""
...
def main():
    ...
    print("W24 G2 — the dry run's captures, at the frozen configuration")
