"""W24 G2 — clause 5's headline: the OKLab ΔE mean per profile, per set, PER TIER.
...
beside it because both of this wave's mechanisms reach it through `optics.ts`'s mirror and its captures move.
...
def main():
    ...
    emit("W24 G2 — OKLab ΔE mean, per profile, per set, per tier, before → after")
    emit("=" * 100)
    emit("before = the canonical matrix at the 0.12.0 landing (the W23 bed); after = the dry run.")
