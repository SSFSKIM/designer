#!/usr/bin/env python3
"""W24 G3: the gate's three derived lists re-derived from the LANDED matrix.
...
attributable rather than asserted.
  ...
  3. **the no-interior scenes** — the dom cells with no `coherence.interiorLevelRatioGpuOverCss`,
    ...
    python3 g3-predicate.py <before matrix>
"""
import json
import os
import re
...
def main():
    ...
    was = excluded(before)
    print("\nwhat the 0.12.0 bed excluded that this one does not, and what joins:")
    index = {}
    ...
    for verb, lines in (('LEAVES', sorted(set(was) - set(machine))),
        ...
        for line in lines:
            print(f"    {verb}  {line}")
            print(f"            0.12.0  {show(before.get(was_index.get(line)))}")
    ...
    print("\n4. the scenes this wave's two mechanisms moved, read on both beds:")
    for scene in ('impulse__capsule-button__rest', 'impulse__rrect-md__rest',
                  'dark-solid__capsule-button__rest', 'dark-solid__rrect-md__rest'):
