#!/usr/bin/env python3
"""W24 G3: every `REGRESSION_FLOORS` entry re-read against the landed matrix.
...
its bound is inert and comes off with its reason — and this landing lights the rim
around every contour on the bed and gives the collapse a transmission, so a floor going inert is
what the wave is FOR and is expected here rather than not.

    python3 g3-floors.py <before matrix>
"""
import json
import os
import re
...
def main():
    ...
    for key, recorded, floor in rows:
        ...
        print(f"{key}\n    floor {direction} {floor}   recorded {recorded}   "
              f"0.12.0 bed {then}   landed {now}   {'held' if held else 'BROKEN'}")
