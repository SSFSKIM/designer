"""W24 G3 by-eye sheets: the edge lit, and the collapse that keeps its transmission.

G2's sheet script, run at the LANDING against the canonical bed — panel 3 is now the canonical
capture this rebuild wrote and panel 2 is the 0.12.0 bed copied to scratch before the rebuild
overwrote it. Nothing else about the composition moves, so the two sheets are comparable to G2's
panel for panel.
...
Four panels per row:
  ...
  4  the CSS tier at the same run, derived from the same document.
    ...
    python3 make-sheet.py --gate g3 --scale 1 \
        --gpu-after <canonical web-captures> --gpu-before <the 0.12.0 bed's web-captures> \
        --matrix <canonical matrix.json> --bed-matrix <the 0.12.0 bed's matrix.json>
"""
import argparse
import json
import os
