#!/usr/bin/env python3
"""W30 G3 — the verdict: every declared row, before and after, in one file.

    python3 verdict.py > verdict.txt

Run BEFORE G1's split script moves the superseded generation out, because that
is the interval in which `results/matrix.json` holds both generations and the
...
`capturePath` carries the material profile document it was captured at and the
first twelve hex of that document's own bytes. The four macOS 27 documents were
re-sealed by `seal.ts` in this child's own commit, so a row captured at the
previous bytes names a hash no file on disk has: "after" is the rows every one
of whose documents is current, which is exactly `atAShippedDocument`'s rule, and
"before" is the rest. Nothing is transcribed and the pair cannot be swapped.

What it prints, in the order the declaration asks for it (claims §5.156 §5 and
`results/2026-09-20-w30-g0-cut/bounds-declaration.md`):

  (c) the three rows this wave CLAIMS, with the lever each is claimed through;
  (d) the four rows declared EXPECTED UNMOVED, where a move is a warning and not
      a result, and the reduced-transparency body-structure residual, which is
      reported on both instruments because they disagree in its direction;
...
CLAIMED = [
    ("dom", "holdout", "checkerboard__rrect-lg__rest",
     "apple-macos-27.0-1x-light-standard-glass0.5", "ssimMean", "≥ 0.9",
     "the shadow's σ, on the CSS tier — span 160"),
    ("dom", "holdout", "checkerboard__glass-over-glass__rest",
     "apple-macos-27.0-1x-light-standard-glass0.5", "ssimMean", "≥ 0.9",
     "the shadow's σ, on the CSS tier — span 130, by extrapolation"),
    ("dom", "holdout", "photo__rrect-lg__rest",
     "apple-macos-27.0-1x-light-reduced-transparency-glass0.5", "ssimOutside", "≥ 0.83",
     "the shadow's σ, on the CSS tier — reachable if the law extrapolates"),
]
...
REPORTED = [
    ("texture", "apple-macos-27.0-1x-light-reduced-transparency-glass0.5",
     "checkerboard__rrect-md__rest", "interiorStdDev",
     "the reduced-transparency body-structure residual — reported, not claimed"),
]
...
def main() -> int:
    ...
    print("W30 G3 — the verdict, before and after, from one read of results/matrix.json")
    print("=" * 108)
    ...
    show("(c) The three rows this wave CLAIMS", CLAIMED, True)
    show("(d) The four rows declared EXPECTED UNMOVED — a move is a warning, not a result",
