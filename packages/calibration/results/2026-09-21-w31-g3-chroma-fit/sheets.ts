/**
 * W31 G3 — the sheets AT THE FIT (Decision Log 2; claims §5.164 §9).
 *
 *   npx tsx results/2026-09-21-w31-g3-chroma-fit/sheets.ts
 *
 * G0's `results/2026-09-21-w31-g3-chroma-fit/sheets.ts`, copied rather than
 * reused on that directory's own convention, with one thing changed: the web
 * panels come from the tree the CANONICAL READ wrote, at the fitted material,
 * so these are the pixels the committed rows beside them were measured off.
 * G0's were pre-fit scratch and said so.
 *
 * Four panels left to right: **native | WebGPU | CSS | x8 difference**, at 1x,
 * in both schemes. The difference panel is the per-pixel OKLab distance between
 * the native fixture and the WEBGPU capture — the tier this wave fits against —
 * scaled by 8 and written as a neutral grey.
 *
 * `eye.md` is written from these, and it is written against G0's `eye.md` cell
 * for cell: the charter's "put the capture next to the native fixture and look"
 * is only a measurement if the two looks are comparable.
 */
...
const SCRATCH = process.env["VITREA_WEB_CAPTURES"] ?? resolve(PACKAGE, "web-captures");
