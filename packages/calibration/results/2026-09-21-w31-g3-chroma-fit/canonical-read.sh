#!/usr/bin/env bash
#
# W31 G3 — the canonical read, at the four documents this child SEALED.
#
...
#
# W30 G3b's `results/2026-09-20-w30-g3b-thin-strip/canonical-read.sh`, copied
# rather than reused, on that file's own rule: it hard-codes the hashes IT
# sealed and refuses any other bytes, and nothing under `results/` is edited
# after commit. The refusal, the exclusivity check, the three machine reads, the
# profile list and the 45-scene ladder are verbatim; the four hashes are this
# child's, filled from `sealed-documents.txt` and never by hand.
#
# **Why there is a read at all, and why it is in this branch.** The four macOS 27
# documents move twice here — once when the digest rule lands and once when the
# chroma retention is fitted into them — and `SHIPPED_DOCUMENT_HASHES` is built
# from their bytes, so every one of the 726 macOS 27 rows falls out of every
# bound the moment they do (X10; W30 Decision Log 4 (b)). The seal and the read
# are therefore one merge, and this is the read.
#
# **The holdout is read ONCE, after `configuration.py record`** (Decision Log
# 1 (b) as clause 6 enforces it). That script prints the four document hashes
# and a hash over the enumerated renderer sources, refuses a second read at
# identical document hashes without a named non-fit reason, and appends to a
# committed log. Run it before the holdout mode below; its output is in the
# ledger.
#
...
LIGHT_SHA=e2fa07589d9984366f36aafb6b8897f4ef978fe592b66d478f3c702ac6a10e50
DARK_SHA=b5714a8662880ff7a70fbfa51a2fa14e3098f11889fcec30edfac6450322bf06
LIGHT_RECEDED_SHA=25863dccef9d62651da4e62cd2c150e021f1cf2859fd2f1769c1dcce018ea1dc
