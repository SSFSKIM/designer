"""W25 G3 — the regression floors, re-read on the dry run beside the 0.13.0 bed.

W24 G3's script on this wave's run. `REGRESSION_FLOORS` in `test/adopted-thresholds.test.ts` is the
...
The wave expected the fourteen THICK floors to move — they are all `checkerboard` rows on thick
surfaces or on the nested pane, which is the family a thick-span change reaches — and the reading
below is which of them did, in which direction, and whether any is breached.
...
def main():
    ...
    table = floors()
    print("W25 G3 — the regression floors, 0.13.0 bed -> the dry run")
    print("=" * 116)
