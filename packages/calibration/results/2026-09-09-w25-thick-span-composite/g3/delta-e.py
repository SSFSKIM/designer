"""W25 G3 — clause 6's headline: the OKLab ΔE mean per profile, per set, PER TIER.

W22 G1's script, carried through W23 G1, on this wave's document. A cell's identity carries the tier, so a mean grouped by
profile and set alone would fold the two tiers together; clause 6 is stated on the GPU tier — the
fidelity target (wave Decision Log 23) — so the two are split here, and the CSS tier is printed
beside it because this wave's one mechanism reaches it NOWHERE and its captures are expected to hold byte for byte.
...
def main():
    ...
    emit("W25 G3 — OKLab ΔE mean, per profile, per set, per tier, before → after")
    emit("=" * 100)
    emit("before = the canonical matrix at the 0.13.0 landing (the W24 bed); after = the dry run.")
    emit("Clause 6 is stated on the GPU tier; the CSS tier, which this wave cannot reach, is beside it.")
    emit("")
    emit(
