"""W25 G3 — the stops S1, S2, S7 and S11, read with numbers.

W23 G1's script by way of W24 G2, on this wave's bed and this wave's one landed constant, the rim's
along-side position field:

  * **S1 — no untinted row worse than the 0.13.0 bed by more than 0.001 ΔE mean or 0.005
    `ssimMean`.** Read per cell off the two matrices, split by tier, because the parent's clause 6
    is stated on the GPU tier. This wave expects the CSS tier's rows to be BYTE-identical, so an S1
    firing there would mean the mirror moved when the arithmetic says it cannot.
  * **S2 — no tinted cell's body moved by more than 0.002.** The field multiplies the rim's
    amplitude and the author's colour sits over it, so a tinted body that moved by more than the
    rim's own reach would say the change went somewhere it should not have.
  * **S7 — no collapsed cell's body moved by more than 0.002.** The field rides `sizeThickness` and
    multiplies the rim OUTSIDE W23's amplitude bracket, so it reaches the collapsed rim as W24's
    exponent does; the collapsed BODY is not a rim and must not follow it.
  * **S11 — W23's straight-span contour reads moved by more than 0.005 on any solid side**, with
    W24's angular reads added to the family by the parent's dispatch (`g3-stops.py`). The field's
    mean over a straight side is exactly zero, so a straight span that moved would mean the mean is
    not zero on the bed; and the angular bins are where the DIRECTION half W24 fitted is checked
    against the POSITION half this wave adds.
...
def main():
    ...
    emit("W25 G3 — the stops S1, S2, S7 and S11, read with numbers")
    emit("=" * 112)
    ...
    emit()
    emit("=== S1 — every row's ΔE mean and ssimMean against the 0.13.0 bed ===")
    ...
    emit()
    emit("  the row the last wave watched S1 on, carried forward for continuity:")
    for ident, cell in sorted(after.items()):
