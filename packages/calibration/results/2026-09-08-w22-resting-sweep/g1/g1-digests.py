"""W22 G1 — the rows the landing must reproduce byte for byte (contract X5).
...
BOTH profile documents are recorded, not only the one that moved. The light document is this wave's
(`optics.regular.specularGain` 0.55 → 0), and the dark one is the record that it did NOT move — its
patch has pinned `specularGain` 0 since W21 G1, so its resolved material is unchanged and a G2 that
reproduces these bytes can say so from this file rather than from the commit message.
...
def main():
    ...
    print("W22 G1 — the dry run's captures, at the frozen configuration")
