"""W22 G1 (3c) — every capture of the dry run against the canonical W21 bed, byte for byte.

The parent's clause 6 says the CSS tier derives from the same profile and its captures are verified,
not assumed. G0 could assume it — the CSS tier draws no specular sweep, so the gate could not reach
it — but W22 G1's constant reaches this tier through `interiorBandLight`, whose term is the CSS
tier's derived interior LEVEL, so its captures are expected to move and each mover has to be named.

Every capture on both tiers is compared: the GPU tier's movers are the fit, and the CSS tier's are
the derived level plus, on the two stacked scenes, W22 G3's backdrop-stack fix, which landed after
the canonical bed was built and moves them on both tiers.
