"""W22 G1 (3b) — clause 4's headline: the OKLab ΔE mean per profile, per set, PER TIER.

`g0/moved-rows.py` groups by profile and set, which folds the two tiers into one mean because a
cell's identity carries the tier. Clause 4 is stated on the GPU tier — the fidelity target (wave
Decision Log 23) — so the two are split here, and the CSS tier is printed beside it because this
wave's constant reaches it through the derived interior level.
...
def main():
    ...
    emit("W22 G1 — OKLab ΔE mean, per profile, per set, per tier, before → after")
    emit("=" * 100)
    emit("before = the canonical matrix at the 0.10.0 landing (the W21 bed); after = the dry run.")
    emit("Clause 4 is stated on the GPU tier; the CSS tier is beside it.")
    emit("")
    emit(
