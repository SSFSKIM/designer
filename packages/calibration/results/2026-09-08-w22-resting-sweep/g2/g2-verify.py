#!/usr/bin/env python3
"""W22 G2: the canonical rebuild read against G1's frozen dry run.

The wave's one holdout read was spent at G1's dry run (contract X5), so this landing is only allowed
to REPRODUCE that render — not to take a second reading of it. The proof is a digest per capture:
G1 recorded one for all 230 render captures at the frozen configuration (`g1-digests.txt`, with both
...
Unlike W21's dry run, G1's passed `--alpha`, so its scratch captures include the declaration-
conformance renders too and the whole 460-file tree is comparable — checked as (ii) beside the 230
the digest file names.
...
The other checks:
  ...
  * **the bed's shape.** 229 cells and the partition `adopted-thresholds.test.ts` asserts, so a
    ...
    per (profile, tier, set) counts printed against the W21 bed's.
...
Derived from W21 G2's `g2-verify.py`, whose shape this is.
"""
import hashlib
import json
import os
...
def main():
    ...
    print(f"canonical: {len(canon)} cells; the W21 bed: {len(before)}; the dry run: {len(dry)}")
    ...
    # ------------------------------------------------- (i) X5 — G1's declared digests
    print("\n(i) X5 — every landed capture reproduces G1's declared digest, byte for byte:")
    declared = {}
    header = []
    ...
    for line in header:
        print('    ' + line.strip())
    live = {}
    for name, path in tree(captures).items():
        live[name] = path
    checked = matched = 0
    voided = []
    missing = []
    ...
    print("\n(iv) the bed's shape:")
    counts = {}
    for profile, renderer, scene in canon:
    ...
    print("\n    per (profile, tier, set), landed against the W21 bed:")
    part = {}
    ...
    for key in sorted(acc):
        ...
        print(f"      {key[0]:44s} {key[1]:8s} {key[2]:12s} n={len(values):3d} "
              f"mean={sum(values) / len(values):.5f}")
