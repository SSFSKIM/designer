...
"""Native | WebGPU | CSS | absolute OKLab difference x8, at the landing (§5.173).
...
AFTER=Path('/Users/new/Developer/GitHub/designer/packages/calibration/web-captures')
...
reader=module('w33_round_reader',CAL/'results/2026-09-22-w33-g1b-rim-fit/read-round.py')
referee=module('w33_sheet_referee',G0/'referee.py')
sealed=json.loads((CAL/'results/2026-09-22-w33-g1b-rim-fit/sealed-manifest.json').read_text())['documents']
...
def load(tree,profile,scene,tier,after):
    ...
    if tier == 'css' and not (directory/f'{scene}__css.png').exists(): return None, []
...
for scheme in ('light','dark'):
    for scale in (1,2):
        ...
        cells += [(p,s) for s in ('hc-text-28__rrect-lg__rest','hc-text-7__rrect-lg__rest',
            'checkerboard-8__rrect-ml__rest','checkerboard-8__rrect-lg__rest',
            'photo__rrect-lg__rest','photo__capsule-button__rest')]
...
for profile,scene in cells:
    ...
    page=Image.new('RGB',(4*width+30,height+125),'white')
    draw=ImageDraw.Draw(page)
    draw.text((5,5),f'{profile} / {scene}',font=font,fill='black')
    for row,(label,tree,is_after) in enumerate((('SHIPPED',AFTER,True),)):
        ...
        for i,(title,image) in enumerate(zip(('Native','WebGPU','CSS','WebGPU vs native: OKLab x8'),
                 (Image.fromarray(n.astype('uint8')),Image.fromarray(w.astype('uint8')),
                  (Image.fromarray(css.astype('uint8')) if css is not None else Image.new('RGB',(width,height),(242,242,242))),delta))):
            x=i*(width+10)
            draw.text((x,y),label+' / '+title,font=font,fill='black')
            page.paste(image,(x,y+25))
        black=referee.black_read(n,w,bg,mask)
        cssblack=referee.black_read(n,css,bg,mask) if css is not None else None
        if css is None:
            draw.text((2*(width+10)+8,y+65),'CSS probe not captured; no substitute.',font=small,fill='black')
        ...
        draw.text((0,footer),f'Black-floor pixels {black["pixels"]}; GPU >0 {black["aboveZero"]}, >1 {black["aboveOne"]}; '+
                  (f'CSS >0 {cssblack["aboveZero"]}, >1 {cssblack["aboveOne"]}' if cssblack else 'CSS unmeasured'),font=small,fill='black')
