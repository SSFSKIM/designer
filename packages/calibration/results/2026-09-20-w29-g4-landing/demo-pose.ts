/**
 * W29 G4, clause 7: what the demo actually does when its window goes to the background,
 * on the macOS 27 material this gate selected (claims §5.155).
 *
 * W28 G4's script (`results/2026-09-15-w28-g4-landing/demo-pose.ts`), re-run at this
 * head with two strings changed. It is re-run rather than cited because the pose it
 * photographs is a different pose: on macOS 26.5 the recede removes the outer shadow
 * and on macOS 27 it keeps it (claims §5.154 §6), and the 27 receded documents are
 * what a root applies from 0.19.0. The instrument is the same so that the two sheets
 ...
 *
 *   tsx results/2026-09-20-w29-g4-landing/demo-pose.ts --out /tmp/w29-g4-demo
 *
...
const report = {
  gate: "W29 G4 — the demo's backgrounded pose, on the macOS 27 material",
  claims: "§5.155",
  scratch: out,
  frontmostBefore,
