"""The sweep-3 reader: the rows this wave turns on, per point, keyed by the
point's OWN profile rather than by grid order — the third form has three axes at
1x and an index-to-combination map is one silent off-by-one away from mislabelled
evidence."""
import json, os, re, sys
sys.path.insert(0, "/Users/new/.claude/jobs/5c70e47f/tmp/w13/sweep-3")
from tab import rows

BASE = ("/Users/new/Developer/GitHub/designer/packages/calibration/results/"
        "2026-09-03-w13-ramp/g0/matrix-x6-baseline.json")
T = "/Users/new/.claude/jobs/5c70e47f/tmp/w13/sweep-3"
...
def tables(name, profileKey, axes, W=78):
    ...
    print("### per-cell ssimBand delta against the W12 close, worst-cell order\n")
    ...
    seq = [("main(W12 close)", None, base)] + [(l, o, r) for l, o, r, _, _, _, _, _, _ in order]
