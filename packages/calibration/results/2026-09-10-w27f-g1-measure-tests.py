"""Tests for the W27f G1 measurement runner, on the failures review found in it.

Stdlib `unittest`, and run directly:

    python 2026-09-10-w27f-g1-measure-tests.py
...
RUNNER = Path(__file__).with_name("2026-09-10-w27f-g1-measure.py")
_spec = importlib.util.spec_from_file_location("w27f_g1", RUNNER)
...
class Scratch(unittest.TestCase):
    def setUp(self):
        self.out = Path(tempfile.mkdtemp(prefix="w27f-g1-test-"))
...
class StackCoverage(unittest.TestCase):
    ...
    def test_the_declared_stacks_of_the_holdout_bed_are_the_two_glass_over_glass_cells(self):
        spec = g1.scene_spec()
        geometry = g1.g0.declared_geometry()
        stacks = g1.stacked_scenes(geometry, g1.declared_scenes("holdout", spec))
        self.assertEqual(stacks, ["checkerboard__glass-over-glass__rest",
