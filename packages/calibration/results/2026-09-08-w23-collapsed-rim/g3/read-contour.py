...
def oklab(linear_rgb) -> tuple:
    """OKLab (L, a, b) of one linear-light colour — the space the hue clause is written in.

    W23 G3's binding is on (a, b): the reference's rim on a painted surface is the paint's own
    colour lifted, and 'lifted' has to be said in a space where lifting is L alone. A chromaticity
    read in sRGB would call an orange whose red channel has clipped a different hue from the same
    orange one code darker, which is exactly the comparison this clause needs to survive.
    """
    lms = OKLAB_M1 @ np.asarray(linear_rgb, dtype=float)
...
def contour_read(lum, rgb, canvas, geom, scale, erode, depth, corner):
    """Body, and per side the contour excess, the first row's excess, the clip fraction — and,
    since W23 G3, the straight-span mean COLOUR of the contour row and of the side's own base,
    in encoded sRGB and in OKLab.

    The colour is meaned in LINEAR light and reported both ways: the encoded triple is what a
    reading by eye quotes and what the parent's crops measured, and the OKLab triple is what the
    hue clause is stated on. Meaning encoded values would make the mean a function of the
    quantiser rather than of the light.
    """
    w, h, radius = geom
    ...
    # `linearise` takes 8-bit values and divides by 255 itself.
    lin = linearise(rgb)
    ...
    for side in SIDES:
        ...
        if hi - lo < 2:
            ...
            out[side] = {"rim": float("nan"), "row0": float("nan"), "base": float("nan"),
                         ...
                         "row0Rgb": nan3, "row0Oklab": nan3, "baseRgb": nan3, "baseOklab": nan3}
            continue
        lines = []
        clips = []
        colours = []
        for k in range(rows):
            ...
            colours.append(cline.mean(axis=0))
        base_lines = []
        base_colours = []
        for k in range(rows, 2 * rows):
            if side == "top":
                base_lines.append(float(lum[int(math.floor(y0)) + k, lo:hi].mean()))
                base_colours.append(lin[int(math.floor(y0)) + k, lo:hi, :].mean(axis=0))
            elif side == "bottom":
                base_lines.append(float(lum[int(math.ceil(y1)) - 1 - k, lo:hi].mean()))
                base_colours.append(lin[int(math.ceil(y1)) - 1 - k, lo:hi, :].mean(axis=0))
            elif side == "left":
                base_lines.append(float(lum[lo:hi, int(math.floor(x0)) + k].mean()))
                base_colours.append(lin[lo:hi, int(math.floor(x0)) + k, :].mean(axis=0))
            else:
                base_lines.append(float(lum[lo:hi, int(math.ceil(x1)) - 1 - k].mean()))
                base_colours.append(lin[lo:hi, int(math.ceil(x1)) - 1 - k, :].mean(axis=0))
        base = float(np.mean(base_lines))
        row0_linear = colours[0]
        base_linear = np.mean(np.stack(base_colours, axis=0), axis=0)
        ...
        out[side] = {
            ...
            "clip": clips[peak],
            "rows": lines,
            # The contour ROW's own colour and the side's base colour (W23 G3): the quantity the
            # hue clause reads, taken on the same straight span as everything above it.
            "row0Rgb": [round(float(v), 2) for v in encode(row0_linear) * 255.0],
            "row0Oklab": list(oklab(row0_linear)),
            "baseRgb": [round(float(v), 2) for v in encode(base_linear) * 255.0],
            "baseOklab": list(oklab(base_linear)),
        }
    return body, out
...
def read_one(path, canvas, geom, erode, depth, corner=1.6):
    ...
    return {
        ...
        "row0Rgb": {s: sides[s]["row0Rgb"] for s in SIDES},
        "row0Oklab": {s: sides[s]["row0Oklab"] for s in SIDES},
        "baseRgb": {s: sides[s]["baseRgb"] for s in SIDES},
        "baseOklab": {s: sides[s]["baseOklab"] for s in SIDES},
...
def main() -> int:
    ...
    for sid in scene_ids:
        ...
        row = {
            ...
            "row0Native": nat["row0"],
            "clipNative": nat["clip"],
            # The hue columns (W23 G3): the contour row's own colour and the side's base colour.
            "row0RgbNative": nat["row0Rgb"],
            "row0OklabNative": nat["row0Oklab"],
            "baseRgbNative": nat["baseRgb"],
            "baseOklabNative": nat["baseOklab"],
        ...
        if args.captures:
            ...
            if os.path.exists(web_png):
                ...
                row["row0RgbWeb"] = web["row0Rgb"]
                row["row0OklabWeb"] = web["row0Oklab"]
                row["baseRgbWeb"] = web["baseRgb"]
                row["baseOklabWeb"] = web["baseOklab"]
