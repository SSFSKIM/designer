#!/usr/bin/env python3
"""W23 G2: the canonical rebuild read against G3's frozen dry run.

The wave's one holdout read was spent at G3's dry run (contract X3; G1's earlier read stands on the
record as spent on a configuration that does not land), so this landing is only allowed to
REPRODUCE that render — not to take a second reading of it. The proof is a digest per capture: G3
recorded one for all 229 render captures at the frozen configuration (`g3-digests.txt`, with both
...
published came from a configuration that no longer exists. That is a stop, not a finding, and it is
checked first.

G3's dry run passed `--alpha`, so its scratch captures include the declaration-conformance renders
...
all four channels, plus whether any differing pixel is in the top two CSS rows inside the contour.
A one-code alpha or edge difference is the capture flake the tracker carries; a rim pixel moving is
...
The other checks:
  ...
  * **the bed's shape.** 229 cells and the partition `adopted-thresholds.test.ts` asserts, so a
    ...
    per (profile, tier, set) counts printed against the 0.11.0 bed's.
  * **the ΔE table** per profile, tier and set on the landed matrix, to be read against claims
    §5.103's dry-run figures.

    python3 g2-verify.py <before matrix> <before captures dir> <g3-digests.txt> <dry-run matrix> \
        <dry-run captures dir>

Derived from W22 G2's `g2-verify.py`, whose shape this is.
"""
import hashlib
import json
import os
...
def main():
    ...
    print(f"canonical: {len(canon)} cells; the 0.11.0 bed: {len(before)}; the dry run: {len(dry)}")
    ...
    print("\n    per (profile, tier, set), landed against the 0.11.0 bed:")
    part = {}
    ...
    for key in sorted(acc):
        ...
        print(f"      {key[0]:44s} {key[1]:8s} {key[2]:12s} n={len(values):3d} "
              f"0.11.0 {prev_s} -> landed {sum(values) / len(values):.5f}")
