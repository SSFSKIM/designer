"""W23 G2 by-eye sheets: the landing — the rim's law, the rim the collapse keeps, and the rim
beneath the paint, on the CANONICAL bed.

G3's sheet script unchanged but for this banner and its inputs: column 3 is now the canonical
`web-captures/` this landing rebuilt rather than a dry run's scratch, and column 2 is the 0.11.0 bed
copied to scratch before the rebuild overwrote it. The frame is the whole canvas at zoom 2 at 1x and
zoom 1 at 2x — the same screen size per CSS px at both scales — and the BLACK-ON-BLACK cells get a
second row beside their own at 4× per CSS px, nearest neighbour, cropped to the top edge: that is
where the whole of the user's finding lives ("not one of our glasses is visible on black, where
Apple's clearly show their presence"), and it is +0.020 of linear luminance on a body of 0.011,
which at the sheet's ordinary zoom is a line nobody can judge.
...
Four panels per row:
  ...
  4  the CSS tier at the same run, derived from the same document.
    ...
    python3 make-sheet.py --gate g2 --scale 1 \
        --gpu-after <canonical web-captures> --gpu-before <the 0.11.0 bed, copied to scratch> \
        --matrix <canonical matrix.json> --bed-matrix <the 0.11.0 matrix>
"""
import argparse
import json
import os
...
LABEL_H = 18
BANNER_H = 44
AFTER_LABEL = {'g3': 'W23 G3 dry run', 'g2': 'W23 G2 LANDED (the canonical bed)'}
...
ZOOMED = (
    # The user's finding: a body one code below its backdrop with a contour rim of +0.020 on it.
    'dark-solid__capsule-button__rest',
    'impulse__capsule-button__rest',
    # And G3's own: the rim's COLOUR on a painted surface, which no luminance clause can see and
    # which at the sheet's ordinary zoom is one line of forty pixels.
    'dark-solid__capsule-button__rest-tint-orange',
    'impulse__capsule-button__rest-tint-orange',
    'checkerboard__capsule-button__rest-tint-orange',
    'photo__capsule-button__rest-tint-blue',
...
def main():
    ...
    banner = ('1 Apple native (the canonical fixture)   2 GPU tier BEFORE (the W22 bed at 0.11.0: '
              ...
              f'the black-on-black and tinted capsules again at {zoom4}x')
