"""W18 by-eye sheets (contract X5): the CSS tier beside the native fixture and beside the GPU tier.

W17's script with its rows and its banner re-aimed at what this wave changes. Five panels per row:
the native fixture; the CSS tier on the W17 bed (the canonical `dom` capture, the outer shadow
drawn as a `box-shadow` on the host and therefore inside the body's own sampled backdrop); the CSS
tier from this wave's captures (the same material with the shadow moved onto L3, or onto the
group's last-painted host where a group has more than one member); the GPU tier on the same bed
(the canonical `webgpu` capture, which this wave binds byte-identical); and a signed difference of
the CSS candidate against native in LINEAR luminance (grey = 0, full scale ±0.25).

The rows are what the wave is about: the three-up at the declared spacing on both structured
backdrops, the stack on both, the lone capsule at the same span as a control, and the middle span
where nothing should move. What to look for is that the shadow OUTSIDE every surface is unchanged
and that no shadow lies on a member's body inside a group (S9). A holdout row whose candidate has
not been read (X8) is skipped and said so.
...
COMPONENTS = {
    ...
    # The two components this wave moves, at their declared extents in `scenes.json`: three
    # 44 x 44 circles at a spacing of 12 (156 wide), and the stack's 220 x 130 base. The margin is
    # widened for both below, because the thing to look at is the shadow OUTSIDE the surfaces.
    'toolbar-group': (156, 44), 'glass-over-glass': (220, 130),
}
# A wider margin on the rows whose subject is the shadow's reach rather than the body's interior.
WIDE_MARGIN = {'toolbar-group': 28, 'glass-over-glass': 24}
...
ROWS = [
    ('checkerboard__toolbar-group__rest', 'toolbar-group'),
    ('photo__toolbar-group__rest', 'toolbar-group'),
    ('checkerboard__glass-over-glass__rest', 'glass-over-glass'),
    ('photo__glass-over-glass__rest', 'glass-over-glass'),
...
def main():
    ...
    for scene, component in ROWS:
        ...
        b = box(component, scale, WIDE_MARGIN.get(component, MARGIN))
