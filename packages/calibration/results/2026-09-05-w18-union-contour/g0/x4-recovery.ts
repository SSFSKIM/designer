/**
 * W18 G0 (f) — contract X4, on this wave's own reader.
 *
 * W17 G0's `x4-recovery.ts` validated `measureCell` under the NATIVE silhouette, which is the mask
 * the bed's numbers are under. This wave reads two further masks that W17's script never exercised:
 * the DECLARED component region, which is the only mask the eight fixture-less scratch scenes admit,
 * and a PER-SURFACE mask, which is one placed shape of a group or a stack. A recovery contract that
 * validated a reader nothing in the findings uses would be a ceremony, so this validates the reader
 * `separation.ts` actually calls, under every mask it actually uses.
 *
 ...
 *   npx tsx results/2026-09-05-w18-union-contour/g0/x4-recovery.ts <scenesJson> <captureRoot> \
