/**
 * W18 G1's pre-check (4) — G0 §6's closed form, run on a whole scene bed.
 *
 * This is `g0/closed-form.ts` (`ebb57fa`) with two changes and no others, and it is copied rather
 * than imported because G0's directory is committed evidence: the shadow is read from
 * `CssTierRender.outerShadow` instead of the host's declaration, because W18 G1 took it off the
 * host, and each row carries its surface's pixel count so that a per-surface prediction can be
 * summed into a whole-cell one. Everything else — the coverage, the chain, the ramp's area mean —
 * is G0's, unchanged, and its residuals against G0's own measurements stand.
 *
 * G0's header follows.
 *
...
/**
 * The `box-shadow` the tier resolves, parsed out of its own declaration string.
 *
 * G0 read this off `render.host["box-shadow"]`, which is where the tier wrote it. W18 G1 took it
 * off the host — that removal is the whole of the closure — and `cssTierDeclarations` now reports
 * the resolved value on `CssTierRender.outerShadow` for whichever carrier paints it. The string
 * and its four numbers are unchanged, so the prediction below is the same prediction; only the
 * field it is read from moved.
 */
function shadowOf(render: { outerShadow: string }): {
  offsetPx: number;
  blurPx: number;
  spreadPx: number;
  alpha: number;
} | undefined {
  const declaration = render.outerShadow;
...
for (const scene of geometryMatrix.scenes) {
  ...
  for (const scale of [1, 2] as const) {
    ...
    placed.forEach((shape, index) => {
      const shadow = shadowOf(renders[index] as { outerShadow: string });
    ...
    placed.forEach((shape, index) => {
      ...
      rows.push({
        ...
        radiusCssPx: shape.radius,
        backdropLinear,
        shadow: shadowOf(render as { outerShadow: string }),
        body: {
        ...
        // The surface's own pixel count under the same mask the prediction is a mean over, so a
        // whole-CELL prediction is an area-weighted sum of its surfaces rather than their average
        // — the canonical bed's components are not all the same size (S5 reads cells, not
        // surfaces).
        pixels: exactCount,
        rampAreaMean: {
