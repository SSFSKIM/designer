#!/usr/bin/env python3
"""W27c G1d: the CSS tier's derivation of the fitted endpoint, for the record only.
...
is one of the terms `tier-coherence.test.ts` pins across the two tiers.
    ...
    python3 css-derivation.py packages/calibration/results/2026-09-14-w27c-g1d/frozen-checking-css-matrix.json \
                              packages/calibration/results/2026-09-14-w27c-g1d/frozen-checking-matrix.json
"""
import json
import os
import sys
...
def main():
    css = json.load(open(sys.argv[1] if len(sys.argv) > 1
                         else "packages/calibration/results/2026-09-14-w27c-g1d/frozen-checking-css-matrix.json"))
    gpu = json.load(open(sys.argv[2] if len(sys.argv) > 2
                         else "packages/calibration/results/2026-09-14-w27c-g1d/frozen-checking-matrix.json"))
