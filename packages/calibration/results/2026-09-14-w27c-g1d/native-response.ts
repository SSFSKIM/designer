/**
 * W27c G1d: the reference's own dark thin response curve, read NATIVE-ONLY
 * (claims §5.143 §2).
 *
 ...
 *     results/2026-09-14-w27c-g1d/native-response.ts
 */
...
const newAnchorRows = rows.filter((row) => row.background === "mid-light-solid");
if (newAnchorRows.length !== 8) {
  throw new Error(`expected 8 mid-light-solid native rows, got ${newAnchorRows.length}`);
}
