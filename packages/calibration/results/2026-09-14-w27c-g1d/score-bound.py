#!/usr/bin/env python3
"""W27c G1d: the SAME declared bound re-applied to the re-read, clause by clause.
...
that has to travel with the number rather than sit in prose somewhere else.
    ...
    python3 score-bound.py packages/calibration/results/2026-09-14-w27c-g1d/frozen-checking-matrix.json
"""
import hashlib
import json
import os
...
def main():
    ...
    frozen = json.load(open(os.path.join(
        REPO, "packages/calibration/results/2026-09-13-w27c-g1c-fit/fitted-endpoint.json")))
    ...
    admissible = {
        patch_digest(frozen["patch"]): "the frozen G1c endpoint (claims §5.141)",
        patch_digest(fitted["patch"]): "the W27c G1d fitted endpoint (claims §5.143)",
    }
    ...
    out = {
        "gate": "W27c G1d re-read / claims §5.143",
