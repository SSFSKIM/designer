#!/bin/bash
# W16 G2: the canonical rebuild at the landing (W16 Decision Log 3, user-decided: land as
# declared; claims §5.73). `main` at the code that ships (the two-layer CSS body: two filtered
# children at the renderer's effective widths, the depth ramp as a raster mask on the heavy one,
# the linear-light reference filter on Chromium, the tint and rim on a third; the GPU tier
# byte-identical by contract X2): build the workspace, keep the previous canonical bed and its
# captures in scratch (they are the W15 bed the landing document compares against), remove
# results/matrix.json, then the twelve per-profile runs to the CANONICAL matrix and the CANONICAL
# web-captures/ (no --out-matrix, no VITREA_WEB_CAPTURES). Derived from W15's g2-rebuild.sh.
set -u
cd /Users/new/Developer/GitHub/designer
unset VITREA_SCENES VITREA_FIXTURES VITREA_WEB_CAPTURES
T=/Users/new/.claude/jobs/5c70e47f/tmp/w16/g2
mkdir -p "$T"
LOG="$T/g2-runs.log"
...
cp results/matrix.json "$T/matrix-before-w16.json"
rm -rf "$T/web-captures-before-w16"; mv web-captures "$T/web-captures-before-w16"; mkdir -p web-captures
