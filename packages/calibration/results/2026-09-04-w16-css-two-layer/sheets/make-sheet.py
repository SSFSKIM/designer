"""W16 by-eye sheets (contract X5): the CSS tier beside the native fixture and beside the GPU tier.

Five panels per row: the native fixture; the CSS tier on the W15 bed (the canonical `dom`
capture, one in-place blur); the CSS tier from this wave's captures (the two-layer body); the
GPU tier on the same bed (the canonical `webgpu` capture, which this wave binds unchanged); and a
signed difference of the CSS candidate against native in LINEAR luminance (grey = 0, full scale
±0.25). The rows are the checkerboard ladder, the hc-text and photo cells at the middle span. A
...
def main():
    ...
    ap.add_argument('--before', default=CANONICAL, help="the W15-bed captures dir (the canonical captures before this wave's rebuild)")
