"""W26 G3 — the regression floors, re-read on the LANDED bed beside the 0.14.0 one.

W25 G4's `g4-floors.py` on this wave's canonical rebuild, unedited but for this docstring.
`REGRESSION_FLOORS` in `test/adopted-thresholds.test.ts` is the list of cells the frozen bed cannot
meet, each pinned at what the bed measured with a floor one `FLOOR_EPSILON` beyond it. A floor comes
off by FIX and is re-pinned only by the user (the fidelity discipline), so this reads every one and
says which way it moved; it never writes the file.

There are ELEVEN floors here, not W25's fourteen: W26 G3a's correction of `silhouetteIoU` to the
decidable region lifted three of them off by fix (claims §5.124; W26 Decision Log 8). What remains
is `checkerboard` rows on thick surfaces and on the nested pane, which is the family a heavy-width
change reaches. A breach stops the landing rather than being pinned.

    g3-floors.py <before matrix.json> <after matrix.json> > g3-floors.txt
"""
...
def main():
    ...
    table = floors()
    print("W26 G3 — the regression floors, the 0.14.0 bed -> the landed bed")
    print("=" * 116)
