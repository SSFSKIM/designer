...
`apply` names its own evidence directory and its own claims sections — no defaults, because
...
positional counter in file order and compares the whole list ordered.
    ...
    python3 split-generation.py apply --evidence <dir> --claims <section> \
        --read-claims <section> [--current <12hex> ...]
    python3 split-generation.py readme
...
W30 G4's invocation, the third application of contract X7, in full:
    ...
    python3 packages/calibration/results/2026-09-20-w30-g1-split/split-generation.py apply \\
        ...
        --claims "c9a §5.160 (W30 G4)" --read-claims "c9a §5.159b (W30 G3b)"

followed by `readme`, which rewrites the table in `results/superseded/README.md` from the
index the apply just wrote and asserts every file's recorded bytes and digest against the
file on disk first.
"""
import datetime
import hashlib
...
# The two tokens the short profile name drops: the OS the whole directory is keyed by and
# the appearance slider's attested position, which every macOS 27 key carries. Dropping
# them is what makes the column readable; nothing is decided from it, and `rowsByProfileKey`
# in `index.json` carries the keys in full.
SHORT_DROP = ("apple-macos-26.5", "apple-macos-27.0", "-glass0.5")


def short_profile(key: str) -> str:
    for token in SHORT_DROP:
        key = key.replace(token, "")
    return key


def write_readme() -> int:
    """Rewrite `README.md`'s file table from `index.json`, and prove the index first.

    The README used to be typed. It said so — "regenerated from `index.json` rather
    than typed" — and it had drifted from the index it named on the one field the two
    both carried (W30 G3/G3b review closure, claims §5.159b §10, finding 8). The table
    is generated here instead, from the index and from the files themselves.

    Nothing about a recorded generation is recomputed: each file's `bytes` and `sha256`
    are ASSERTED against the file on disk and the run refuses if either has moved,
    because this tool's whole premise is that a superseded row is never rewritten.
    """
    index_path = SUPERSEDED / "index.json"
    readme_path = SUPERSEDED / "README.md"
    index = json.loads(index_path.read_text())
    files = index["files"]

    for name, entry in sorted(files.items()):
        raw = (SUPERSEDED / name).read_bytes()
        if len(raw) != entry["bytes"] or hashlib.sha256(raw).hexdigest() != entry["sha256"]:
            print(f"{name}: the file on disk is {len(raw)} bytes / "
                  f"{hashlib.sha256(raw).hexdigest()[:12]} against the index's "
                  f"{entry['bytes']} / {entry['sha256'][:12]}; refusing")
            return 1

    rows = [
        "| file | profiles | active document read at | read under | moved under | "
        "captured | superseded | rows | bytes |",
        "| --- | --- | --- | --- | --- | --- | --- | ---: | ---: |",
    ]
    for name, entry in sorted(files.items(), key=lambda kv: kv[1]["capturedAt"]["first"]):
        profiles = ", ".join(
            f"`{short_profile(key)}` ({count})"
            for key, count in entry["rowsByProfileKey"].items())
        active = next(d for d in entry["documents"]
                      if d["sha256"] == entry["activeDocumentSha256"])
        receded = [d for d in entry["documents"] if d["sha256"] != entry["activeDocumentSha256"]]
        document = f"`{pathlib.Path(active['path']).name}` `sha256:{active['sha256']}`"
        if receded:
            # The OS token elided, the appearance slider's kept: the second is part of
            # which material this is and the first is the whole directory's subject.
            name_ = pathlib.Path(active["path"]).name
            for token in ("apple-macos-26.5", "apple-macos-27.0"):
                name_ = name_.replace(token, "…")
            document = (f"`{name_}` `sha256:{active['sha256']}`, with `-receded` "
                        + ", ".join(f"`sha256:{d['sha256']}`" for d in receded))
        rows.append(
    ...
    readme_path.write_text(
        text[:start] + README_BEGIN + "\n\n" + "\n".join(rows) + "\n\n" + text[end:])
    print(f"wrote {readme_path.relative_to(ROOT)}  {len(files)} files, every byte count and "
          f"digest checked against the file on disk")
    return 0
...
def main() -> int:
    ...
    while i < len(argv):
        ...
        if a == "--current":
            ...
            while i < len(argv) and not argv[i].startswith("--"):
                ...
                i += 1
            continue
        if a in ("--evidence", "--claims", "--read-claims"):
            if i + 1 >= len(argv):
    ...
    if malformed:
        ...
        return 2
    if mode == "apply" and (evidence is None or claims is None or read_claims is None):
        print("apply requires --evidence <dir>, --claims <section> (the gate that MOVES "
              "the rows) and --read-claims <section> (the gate that READ them); see the "
              "docstring for G4's invocation")
        return 2
    ...
    index = {
        ...
        "claimsFields": (
            "`readUnderClaims` is the claims section of the gate whose read produced "
            "these rows; `movedUnderClaims` is the section of the gate that superseded "
            "them and moved them here. They are never the same gate. Split in two on "
            "2026-09-20 (c9a §5.159b §10, finding 8), when one field named `readUnderClaims` "
    ...
    for stem in sorted(dest):
        ...
        files[f"{stem}.json"] = {
            ...
            "readUnderClaims": read_claims,
            "movedUnderClaims": claims,
    ...
    # The README's table is the index rendered, so it is written by the run that
    # writes the index and never by hand.
    return write_readme()
