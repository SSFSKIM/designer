...
- **Eval iteration 1 (2026-07-24): the skill's failure mode is build-time drift from its own written law, not weak law.** Across 6 briefs the skill lifted a Sonnet executor from 43% to 94% assertion pass-rate (+0.51). Divergence (eval-5 terminal vs risograph under two seeds) and brief-authority (eval-6 honoring an explicitly-requested cream/serif/terracotta look) both confirmed. The two with-skill misses were the SAME class: the page contradicted rules its own DESIGN.md declared — a repeated eyebrow-kicker (eval-3) and a cream-band ground + stray `rgb()` literals (eval-5 seed11) — both after QA prose *claimed* compliance. Implication for the QA/verification step: replace prose self-attestation with an actual mechanical check of the built page against its DESIGN.md law (kicker-repeat grep, color-literal-outside-`:root` grep, background-token OKLCH re-check). Baselines meanwhile fell into cream grounds, repeated kickers, and scattered hex repeatedly — direct evidence the anti-slop layer earns its place. (Full detail: `.doperpowers/sdd/task-19-analysis.md`.)
- **Iteration-1 gate feedback (user, 2026-07-24):** approved the results; applied the build-time-enforcement fix (commit a45dbb3). But the honest headline note was **"it still has an AI-feeling"** — passing assertions and committing to a stance is not yet the same as *not reading as AI*. The two fixes target two concrete tells (repeated kicker, cream-band drift) but the broader feeling runs deeper. **Deferred to eval iteration 2 as its primary goal**, to be diagnosed against specific outputs the user points at (which tells, in which pages) rather than a blind "make it less AI" edit — the skill-creator caution against overfitty guessing applies. Candidate levers to probe there: is the "spend boldness in one place / one signature element" discipline actually landing, or is every output uniformly tasteful (a flatness that itself reads as AI); is committed-but-generic *content* a bigger driver than visual choices.
- **Future eval idea (user, deferred):** run the same with/without benchmark but comparing `figma-design` against `frontend-design` and `impeccable` as the baselines (not no-skill), to see where each of the three-source foundation actually wins. Connects directly to the project's second act (the SVG-filter skill built on all three). → **Ran 2026-07-24, see next entry.**
- **Three-way comparison eval (2026-07-24): user verdict — figma-design ≈ impeccable in quality, frontend-design one clear level below; cost-adjusted, figma-design wins.** 12 runs (4 briefs × 3 skills, all Sonnet, same self-contained-HTML contract, no live browser for any arm, human-judged via a side-by-side grid in `figma-design-workspace/comparison/`). The user judged figma-design and impeccable evenly matched ("엄대엄") while impeccable spent ~2–3× the tokens — and impeccable's headless run *skipped* its most expensive phase (live iteration) and was the only arm to break the self-contained contract (3 Unsplash photos on the meditation brief). Cross-skill patterns: heavy concept convergence (all three landed a "stamp" motif on the kids-library brief; all three went industrial on the hardware brief), light-vs-dark split on the rail dashboard (figma-design alone chose a light ground), and all three honored the explicit cream/serif/terracotta meditation brief. The convergence supports the triple-convergent-evolution observation above — the shared anti-default taxonomy drives concepts more than any one skill's method. Caveats: 1 run per cell, Sonnet-only, eyeball judgment — evidence for "competitive with the best at a third of the cost," not yet general superiority. The new verify-against-own-law QA step (commit a45dbb3 / 0f107f1) fired in the wild in two of the four figma-design arms, catching real violations pre-delivery.

- **Font-diversity audit (2026-07-25), triggered by the user's observation during the vs-real-Figma-Make eval that our outputs' fonts/libraries feel less varied than Make's.** Three-source audit (this skill, impeccable, the interview corpus) found the pool itself is a faithful, wide Make port (~60 Google-Fonts faces + 13 traditions) — the narrowness comes from three leaks: **(1)** the tradition→loadable-face seam is unbridged (sampler exemplars are mostly commercial faces; nothing maps them to the Google-Fonts pool, so the step falls through to model memory); **(2)** the "ten strong pairings" section acts as a de-facto menu (Fraunces appeared in 3 of 4 comparison briefs; the guesthouse arm shipped pairing #2 verbatim, stance label included); **(3)** the overused-defaults table is one generation stale — impeccable's ban list already flags the newer Geist/Instrument/Recoleta/Fraunces monoculture that our skill *features*. For contrast, impeccable ships no positive pool at all (ban-list-only, "browse a catalog" — inoperable headless), and real Make's extra perceived richness beyond fonts is largely environmental (48 shadcn/Radix components, lucide icons, Recharts). **Leak 2 fixed (commit c4513c9, user-approved): pairings retitled "worked pairings — method illustrations, not a menu" with a binding derive-first / compare-second / swap-one-slot order. Leaks 1 and 3 deferred pending user go.**

- **Final eval vs real Figma Make (2026-07-25): user verdict — figma-design wins 2 of 3.** Three fresh briefs (energy dashboard / film-festival microsite / guesthouse booking), our side run on Opus workers with real Unsplash images, Google Fonts, and live browser QA permitted (leveling the field against the real product); the user ran the same prompts on Figma Make's website and judged side-by-side (`figma-design-workspace/comparison-vs-make/`). On the energy dashboard the user called ours "clearly better" — though Make's output reported a code error, so that cell is a compromised comparison (Make ships into a runtime with a fix-error affordance; ours shipped working first-render code, itself a data point for the QA discipline). Of the remaining two, figma-design took the overall series 2–1. The user declared the skill **fully done**: "Figma-design skill 제작 완료" — this closes deliverable #1 and green-lights deliverable #2 (the SVG-filter skill). Caveats as ever: 3 briefs, 1 run per cell, eyeball judgment; and the user's font-diversity observation from this eval (see previous entry) already produced the Leak-2 fix, with Leaks 1 and 3 still deferred.
...
- ~~The user's honest note that outputs "still have an AI-feeling" is the primary goal for eval **iteration 2** — to be diagnosed against specific outputs the user points at, not guessed at with a blind edit.~~ Dropped by the user 2026-07-25 ("no need to diagnose residual AI-feeling") after the vs-real-Make eval settled the capability question (2–1 for figma-design; see Surprises & Discoveries).
- ~~A three-way comparison eval (figma-design vs frontend-design vs impeccable as the baselines, rather than no-skill).~~ Done 2026-07-24 — user verdict: figma-design ≈ impeccable at ~⅓ the tokens, frontend-design a level below (see Surprises & Discoveries).
- ~~Minor findings logged per-task in `.doperpowers/sdd/progress.md` (sampler NaN-seed validation, a `stances.md` footnote citation, etc.) for a cleanup pass.~~ Done 2026-07-24, commit 0f107f1.
...
- 2026-09-05: the stance-layer doctrine in this spec (five complete systems ported intact; the as-is / variant / derive-fresh mapping) is superseded by `2026-09-05-stance-derivation.md`, after an audit found the same accent hex shipping across four unrelated products. The five systems survive as worked derivations; every value is now derived per product.
