...
- **X8 — Shape channel set** (§Geometry; binds C3, C6, C8): `{ center, size, radii, smoothing, thickness }` — every morph channel numeric. S2 confirmed the set unchanged, with three riders: smoothing is budget-clamped (continuously) by size and radii; thickness needs no separate bound because the concentric shape is a level-set offset of the same field; radii are uniform-only in v1 (Vec4 shape preserved). C6's instance buffer widens by six derived floats.
