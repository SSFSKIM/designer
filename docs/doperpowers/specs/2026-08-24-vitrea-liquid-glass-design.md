...
| C9 fidelity, demo & release | decomposed 2026-08-25 (see §C9 closing cut) | C9b landed (17 demo e2e; zero diagnostics; static deploy verified); C9a in-flight; C9c blocked-by C9a; C9d blocked |
...
- **C9a — fidelity tuning** (opus, in-flight): extend the comparison to the full scene matrix, tune the calibration-delegated constants (material tunables, union caps, motion where measurable) against calibration+validation cells, verify against holdouts untouched, write per-tier claims. Inherits C7's three scoping items (thickness field, mixed-backend glass-over-glass cell, pressed-pose caveat). External sub-gates: a11y-profile captures, optional 2x run.
