...
- **Status:** G0 closed; G1 fitted and declared 2026-09-10 (claims §5.130), reviewed by the
  ...
  over sets whose footprints differ 3.4–3.7×; on the interior mask the light and accessibility
  holdouts are at or below their own validation level and all 30 spent cells clear the active
  bed's adopted ceiling, while the dark pair is genuinely 3.5× worse on two cells. G2 (the
  ...
  freeze bar and 11–15 h with attempt loss rather than Decision Log 5's one to two, and the OS 27
  bed cannot check the 26.5 endpoint because 27 changes the material.
...
- 2026-09-11, **W27c G1b's analysis half closed, and three recorded figures it could not
  ...
  **(i) Decision Log 12's premise.** "Seven to eight times worse on holdout" is computed on the
  ...
  across sets whose mean footprint differs 3.4–3.7× because the inactive split mirrors an active
  ...
  two cells carry it. All 30 spent holdout cells clear the `oklabDeltaEMean` ceiling the **active**
  material is already held to, the worst at 69% of it. The hold's *direction* survives — the dark
  endpoint is unproven, no inactive floor is adoptable, and four named model-form gaps are open —
  but "a pose proven on 23 cells only" is not what the light and accessibility readings say. **No
  gate is unblocked by this note**; the ruling is the user's and G2 and G3 stay held.
  **(ii) Decision Log 5's capture estimate.** "One to two hours of the user's machine" matches no
  ...
  freeze bar and 11–15 h once attempt loss is budgeted, and W25 G1's comparable two seven-run
  passes ran 5 h 40 min. Decision Log 5 also called a later attested session "available later
  ...
  **(iii) The stacking sentence** in claims §5.130 §7 is withdrawn: the stack's overlay resolves
  ...
  the *active* flat white at α 0.665 — so the inactive endpoint never reached the second plane, and
  W27f G1 replaced that flat on `main` after those rows were captured. Two of the 30 holdout rows
  are stale against today's runtime; the other 28 are not.
