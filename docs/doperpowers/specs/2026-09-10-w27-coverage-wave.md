...
- **Status:** G0 CLOSED 2026-09-11 (claims §5.133). The corpus is 57 dumps and 60 occurrences in 4
  ...
  **The 1x both-pose pass is READ 2026-09-13 (claims §5.138; evidence
  `results/2026-09-13-w27e-probe-1x-reading/`), and none of §5.137 §6's four outcomes fits.**
  ...
  **key** rather than with deactivation, the 16 dumps whose application stayed active being
  ...
  standing beside this). §5.136 §5's remaining axis is therefore the **scale**: the 2x corpus is in
  the same receded configuration and differs on 10 of 50 cells, every one of them a cell that adapts
  at 1x and not at 2x, with `tracksLuma` identical at both scales so a device-pixel size gate is
  refuted and the mechanism unread. **No selector is declared and Decision Log 15 (c) stays the
...
**W27e G2's gaps to macOS (2026-09-13, claims §5.138 §10).** Four, none of them a pixel measurement,
because this reading captured nothing. *How the backing scale suppresses the body's adaptation is
unread:* at 2x no cell in the probe bed adapts in either scheme, and the obvious explanation — a size
gate in device pixels — is refuted by `tracksLuma`, which reads 1 at declared span 44/48/64 and 0 at
80/96 at **both** scales. The separating experiment is cheap and named: a 2x `dark-solid` ladder at
declared span **22 / 32 / 44** through `dump-layers`, which captures no pixels and needs no fixture;
if the adaptation returns at the smaller declared spans the gate is in device pixels after all and
`tracksLuma` is not what gates it. *Both of Apple's adaptation thresholds are brackets with no cell
inside them* — the light-scheme span gate in (64, 80] declared points and the dark-scheme tone
threshold in (0.5, 0.890969] — and vitrea models the first (its fitted knee sits inside Apple's
interval) and the second with nothing at all: `backdropToneAdaptation` reads 0 on every dark cell that
switches. Closing that needs a tone ladder between linear 0.5 and 0.891 at one span in dark, and it is
a **fit**, so it needs a declared partition and a holdout, not another read. *Whether the two non-key
states are the same pose in pixels has never been measured:* they agree on every configuration field
§5.138 reads, but no capture compares `.accessory`-never-activated against regular-policy-key-lost,
and §5.136 §1 chose the capture's mechanism on the pixel side. *And the run-to-run instability of the
body's adaptation* is in `specs/tech-debt-tracker.md` with the settle study that would close it.
...
- 2026-09-13, **the 1x both-pose reading TAKEN: the window state moves the layer, not the
  ...
  **None of §5.137 §6's four outcomes fits, and a fifth reading does.** The pose does not select the
  ...
  **That leaves the scale as §5.136 §5's axis.** The 2x corpus sits in the same receded
  configuration (non-key, margin 0, highlight opacity 0), differs from the 1x recede on 10 of 50
  surface cells and 5 of 24 labels, and every difference is a cell that adapts at 1x and does not at
  2x. `tracksLuma` is identical at both scales, so a device-pixel size gate is refuted and the
  mechanism stays unread. `active/light`'s 16 non-key dumps are the recede reached by losing key
  alone, agreeing with the accessory arm on 16 of 16 scenes and on every field — a second,
  independent route to the receded configuration, and not a claim about pixels.
  ...
  **The gap, logged not closed (tracker):** three cells carry two different operators across runs of
  ...
  reading stands as recorded and this is the second beside it. Any selector law is bounded from above
  by it.
  ...
  **The recommendation for Decision Log 15 (c), which is NOT taken here.** *Scheme-only replaces
  ...
  beside it — the crossover is not a vitrea-only compromise but the same quantity Apple switches on,
  so one selector is right and two would be the disagreement §5.137 §5 warned of. *What the reading
  says instead:* **`foregroundCrossover` keeps the role and gains Apple's authority**, and the open
  question is no longer *what* the selector reads but *where its threshold sits* — Apple's own is
  scheme-dependent and this corpus brackets it only as (64, 80] declared points in light and
  (0.5, 0.890969] in tone in dark, which is far too wide to adopt, so vitrea's fitted 0.475 stays
  where it is and the divergence is recorded rather than tuned. The user's call is whether to take
  that third reading, and whether the threshold question is chartered now or deferred.
