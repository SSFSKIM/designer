...
- 2026-09-13, **W27e G3's review fix wave CONVERGED: the corrected run taken, §2 re-derived from the
  ...
  rows per cell whose reached offsets sit within **4 ms** of 400 / 2200 / 4200 / 6200, where the old
  cumulative scheme drifted with every screenshot it took. Thirteen of §2's twenty-nine rows move and
  ten of those because the family's minimum now falls on a phase nobody had sampled: the site's
  dark-scheme selected favourite falls 8.690 → **7.010** CSS and 9.332 → **8.140** WebGPU, and the
...
- 2026-09-13, **W27e G3's independent review: six findings applied, and the eye held back until a
  ...
  they are now absolute offsets from each scenario's start, with the offset actually reached
  recorded, which `contrast.spec.ts` and `page-stage.gpu.spec.ts` inherit through `worstRatio`. Six
