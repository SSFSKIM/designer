...
- **Status:** G0 completed (claims §5.129); G1 completed (claims §5.131, 2026-09-10) and merged
  ...
  tripped as declared and is outstanding**: one record-only CSS capture is not byte-repeatable, and
  narrowing a declared stop after the read is the user's ruling under Decision Log 13, so the gate
  recorded it rather than deciding it. **The user's acceptance of `/#page` is still not
...
- 2026-09-11, **W27f G2 closed; the wave's last landing gate.** Claims §5.135 and
  ...
  adopted — with instrument stop **S4 tripped as declared and outstanding**: one record-only CSS
  capture is not byte-repeatable, `declaration.md` §6 worded S4 without scoping it, and narrowing a
  declared stop after the read is the user's ruling under Decision Log 13, not a landing gate's.
  The gate's X1 argument for reading S4 narrowly is on the record as a recommendation.
