...
| 0.23.0 | **Prepared, unpublished; complete chain recorded at §5.173.** Evidence `packages/calibration/results/2026-09-22-w33-g2-landing/`: `chain.sh`, `chain-resume.sh`, `chain-status.txt`, per-step logs, `browser-runs.txt`, `dry-run.sh`/`.txt`. Six non-browser steps completed before a temporary eye stop; raw two-axis pixels falsified the apparent stripe defect and only the unrun browser tail resumed. Freeze **1,818** open/close; tree **1,900 /1,893 match /0 mismatch /7 no-row**; build/lint/root eslint green; units **2,719 /189 files** (calibration648, including four X1 cases and its live pixel derivation); goldens **34**, byte-identical; GPU **48**; platform **410**; React **174 pass /3 skip**, first and only run, no timing-red rerun. The normal demo command ran **zero tests**, setup exit1: an orphaned Vite from the removed W32 G2 worktree occupied5177. The parent stopped it after the single isolated-port continuation had started, so the suite uses `demo-isolated.config.ts` on5197 with normal tests/projects and no server reuse; it passed **61/61 in9.9min**, including all10 `/laws/` cases; final freeze **1,818** after it. Gated **230/786**, frozen **229/1,107**, predicate67. X1 adopts zero >0 fraction and >1 count on **218** standard single-shape non-holdout cells; G0's232 remains the broader referee. M2 **26/26**, unchanged2% bound. X6's **10 readings** preserve one refused own-shell false positive before launch and nine eligible attempts, all RT0/IC0/slider0.5/foreign0, idle≥60.065s. Version generated by `pnpm changeset version`, every changeset consumed. Rehearsal clean: packed0.23.0, ranges **^0.23.0**, required files and dist verified; core **583,802 B**, web **568,292 B**, React **188,179 B**. Four resolved digest prefixes **dcbccbd9feac9881 /e59f9106bcd7c966 /f34dcc03e2774db3 /6b6237b7ae241638**, file SHA prefixes **6e509c7f76cc /eab099cc6698 /45acb6d916b9 /4e68f81869f6** in light-active/dark-active/light-receded/dark-receded order; both frozen26.5 digests remain **b2b570e4adcea8fb /874be66ea501621b**. No material or document reseal at G2. The rehearsal does NOT invoke `changeset publish`; its last live exercise stays0.1.0 with CLI3. *(Corrected beside at the G2 review closure, 2026-09-22: `pnpm release` is `pnpm -r build && changeset publish`, so the user's release of 0.22.0 on `c46eeba3` — and 0.21.0 before it, both recorded as `pnpm release` — DID invoke `changeset publish`; what this rehearsal does not exercise is that step. The 0.21.0 row's "not invoked since 0.1.0" is carried as recorded, unverified here for the releases before 0.21.0.)*0.1. Review, merge and the user's `pnpm release`, then tag, remain later actions. **Reviewed and merged as `e6c80c7f` 2026-09-22 (closures `d26a2fa9`, `75435daf` on the branch; freeze 1,818; chain re-run on main green, 2,719 units). Published 2026-09-23 on `e6c80c7f`, tag `v0.23.0` pushed; cold install verified in a fresh `mktemp -d` directory (core 44 / web 258 / react 38 exports, ranges `^0.23.0`, the four macOS 27 digests and the 26.5 pair reading their own fields, `liftAmplitude: 0` with `spreadPx` 0.5 / 1.8 and `sigmaThinOffsetPx` -6.318 in a 303,207 B web bundle); the three tarballs fetched back from the registry are 583,802 / 568,292 / 188,179 B, byte-for-byte this row's rehearsal sizes; the registry listed the whole group in 3.0 s (core 00:05:49Z, web 00:05:51Z, react 00:05:52Z) and served every tarball on the first install two minutes later, so neither 0.21.0's two-layer window nor 0.22.0's 23.6 s spread recurred (W33 charter Status).** |
| 0.24.0 | **Prepared, unpublished; complete qualified chain at §5.180.** Evidence `packages/calibration/results/2026-09-24-w36-g2-landing/`: ordered `chain.sh`, per-step exits/logs, nine X6 preflights, dry-run and independent tarball check. Freeze **1,818** at open/close and after eye; canonical tree **1,900 /1,893 match /0 mismatch /7 no-row**, all786 live27 rows at G1's hashes. Build/lint/root eslint green; units **2,761 /196 files** (policy23, motion164, geometry170, renderer566, core302, platform634, React174, calibration682, demo46); goldens **34**,13 PNGs byte-identical; GPU **49**; platform **410**. React **173 passed /3 skips /1 failed**: standing Firefox morph-release case, destination still mounted5s after Escape; first and only run, retained, not rerun or called green. Demo **61** on free normal5177; the later black-control/prose correction has its own expected red then **10 laws passes**, demo build/lint/46 units and root eslint green. Gated **230/786**, frozen **229/1,107**, predicate **67**, named misses **8**. L1 adopted on140 standard WebGPU cal/val rows:136 measured, four UNMEASURED, two unfloored0.066 misses; growth0 against0.005, all three scratch discriminations red. M2 reference unchanged from G1,26 zero wave increments, cumulative −2.774796%…+1.965563%. All four X6 facts hold, idle≥43,325.054s; foreign counts seven0/two1 (owned Vite-launch shell, raw census retained); live eye hardware and byte-deterministic.24 guarded triptychs and live black demo viewed; page target and label-free RGB132 agree, no geometry-equality claim. Version generated from G1's changeset, fixed group0.24.0. Dry rehearsal exit0, required files/dist checked, tarballs **586,228 B core /569,814 B web /188,606 B React**, dependent ranges **^0.24.0**. Resolved digests **be13dae45098fc89 /2a4323f33df8d799 /b0d0d8dacc6a03af /7c454858a3cbad5b**, file hashes **85ad7f7e3e0d /0eac5b294cc2 /30fbe05986ae /5cec8c961201**, in light-active/dark-active/light-receded/dark-receded order; frozen26.5 pair **b2b570e4adcea8fb /874be66ea501621b** unchanged. No material change, reseal, native or holdout read at G2. This rehearsal does not invoke `changeset publish`; no merge/push/publication/tag. Independent review and the user's sanctioned `pnpm release` remain later, with the timing red disclosed. **Reviewed and merged as `e27880a1` 2026-09-24 (freeze 1,818; review with no material finding, the timing red named as the one disclosure). Published 2026-09-25 (KST) on `e27880a1`, tag `v0.24.0` pushed; cold install verified in a fresh `mktemp -d` directory (core 44 / web 258 / react 38 exports, ranges `^0.24.0`, the four macOS 27 digests and the 26.5 pair reading their own fields, neither 0.23.0 digest present, `backdropToneBlackStrength: 1` with its four ordinate pairs in a 304,477 B web bundle); the three tarballs fetched back from the registry are 586,228 / 569,814 / 188,606 B, byte-for-byte this row's rehearsal sizes, their SHA-1s matching the registry's; the registry listed the group over 88.4 s (core 23:56:26Z, web 23:57:17Z, react 23:57:54Z) and served every tarball on the first cold install four minutes after the last listing — a wider spread than 0.23.0's 3.0 s or 0.22.0's 23.6 s, and the parent's watch called it partial one second early (W36 charter Status and Revision Notes).** |
