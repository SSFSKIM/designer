...
re-score, against `main` at `d3d3163`. **Eight rows move and one row is added.** The 2026-09-10
table above is left as written and no row's earlier record is rewritten.
...
| `GlassButtonStyle` / `GlassProminentButtonStyle` | §1.6 | `partial` | **`replicated+measured`** | The row's recorded blocker — `tint` absent from `GlassButtonProps`' allow-list — is closed by W27a: `GlassButton` and `GlassIconButton` forward `tint` and `foreground` (`packages/react/src/controls/button.tsx`), measured on the tinted capsule-button cells `photo__capsule-button__rest-tint-orange` and `checkerboard__capsule-button__rest-tint-blue`, and shown live in `apps/demo/src/TintInkPlate.tsx`. **The prominent form is still absent** and is now held by a recorded ruling, W27 Decision Log 6, rather than unruled. §1.6's `UIButton.Configuration` row keeps `partial`, because the two prominent forms are the whole of its gap |
...
| layer | rows | `replicated+measured` | `replicated, unmeasured` | `partial` | `excluded by decision` | `absent, undecided` | n/a |
| --- | --- | --- | --- | --- | --- | --- | --- |
| §1 Material API surface | 52 | 14 → **15** | 4 → **7** | 8 → **6** | 4 → 4 | 12 → **10** | 10 → 10 |
| §2 Component families | 26 | 1 → 1 | 3 → 3 | 3 → 3 | 7 → 7 | 10 → 10 | 2 → 2 |
| §3 Behavioral system | 96 → **97** | 26 → **29** | 18 → **19** | 15 → 15 | 12 → **11** | 18 → **16** | 7 → 7 |
| **total** | **174 → 175** | **41 → 45** | **25 → 29** | **26 → 24** | **23 → 22** | **40 → 36** | **19 → 19** |

As proportions of the **156** scoreable rows (155 plus the row added under §3.7): replicated and
measured 26% → **29%**; replicated, unmeasured 16% → **19%**; partial 17% → 15%; excluded by
decision 15% → 14%; absent and never ruled on 26% → **23%**. §3's `partial` count is unchanged at
15 and its composition is not: materialize left it upward, while window focus entered it from
...
the project. The other moves are W27a's button tint, W27b's toolbar partition, W27e's operator and
named ink levels, and W27f's page material. Clause 1 also named window focus as
...
**The eight rows W27 built and this re-score did not move.** §2's *Toolbars / toolbar items* keeps
